...
Matrix<double, 30, 256> W1;
Matrix<double, 30, 256> dW1 = MatrixXd::Zero(30, 256);
Matrix<double, 5, 30> W2;
Matrix<double, 5, 30> dW2 = MatrixXd::Zero(5, 30);
VectorXd b1 = VectorXd::Zero(30);
VectorXd b2 = VectorXd::Zero(5);
VectorXd x = VectorXd::Zero(256);
VectorXd y = VectorXd::Zero(30);
VectorXd z = VectorXd::Zero(5);
VectorXd u1 = VectorXd::Zero(30);
...
int main(){
	...
	for(i = 0; i < loop; i++){
		...
		d(sp) = 1;
		delta2 = z-d;
		VectorXd fPrime = VectorXd::Zero(30);
		for(j = 0; j < 30; j++){
			if(u1(j) > 0){
				fPrime(j) = 1;
			}else{
